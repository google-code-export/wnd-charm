...
double contrast(ImageMatrix *image) {
	...
	for (x = 0; x < image->width; x++) {
		for (y = 0; y < image->height; y++) {
			vec[x*image->height+y] = image->pix_plane(y,x);
			avg += image->pix_plane(y,x);
		}
	}
	...
	for (x = 0; x < image->width; x++)
		for (y = 0; y < image->height; y++)
			std += (image->pix_plane(y,x) - avg)*(image->pix_plane(y,x) - avg);
...
double efficientLocalMean(const int x,const int y,const int k, ImageMatrix *laufendeSumme) {
	...
	else links = laufendeSumme->pix_plane(stopy,startx-1);

	if (starty-1 < 0) oben = 0;
	else oben = laufendeSumme->pix_plane(stopy-1,startx);

	if ((starty-1 < 0) || (startx-1 <0)) obenlinks = 0;
	else obenlinks = laufendeSumme->pix_plane(stopy-1,startx-1);

	unten = laufendeSumme->pix_plane(stopy,startx);
...
double coarseness(ImageMatrix *image, double *hist,int nbins) {
	...
	for(y = 0; y < (int)yDim; ++y) {
		for(x = 0; x < (int)xDim; ++x) {
			if(x < 1) links = 0;
			else links = laufendeSumme->pix_plane(y,x-1);

			if(y < 1) oben = 0;
			else oben = laufendeSumme->pix_plane(y-1,x);
	...
	for(y = 0; y < (int)yDim; ++y) {
		for(x = 0; x < (int)xDim; ++x) {
			...
			for(int k = 1; k <= K_VALUE; ++k) {
				if(Ekh[k-1]->pix_plane(y,x) > maxE) {
					maxE = Ekh[k-1]->pix_plane(y,x);
					maxk = k;
				}
				if(Ekv[k-1]->pix_plane(y,x) > maxE) {
					maxE = Ekv[k-1]->pix_plane(y,x);
					maxk = k;
				}
			}
